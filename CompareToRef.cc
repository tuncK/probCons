...
int main(int argc, char **argv) {
  ...
  // check arguments
  if (argc < 3) {
    cerr << "Usage: score TEST_ALIGNMENT REFERENCE_ALIGNMENT "
            "[BALIBASE_ANNOT_FILE] [-col] [-core] [-caps] [-annot FILENAME]"
         << endl;
    exit(1);
  }
  ...
  if (infile.fail()) {
    cerr << "ERROR: Could not open file '" << argv[1] << "' for reading."
         << endl;
    testAlign = NULL;
  } else {
    testAlign = new MultiSequence();
    assert(testAlign);
    testAlign->LoadMFA(infile);
  }
  infile.close();

  MultiSequence *refAlign = new MultiSequence(string(argv[2]));
  assert(refAlign);
  ...
  if (testAlign) {
    for (int i = 0; i < testAlign->GetNumSequences(); i++) {
      ...
      for (int j = 0; !found && j < refAlign->GetNumSequences(); j++) {
        if (testAlign->GetSequence(i)->GetHeader() ==
            refAlign->GetSequence(j)->GetHeader())
          found = true;
      }
    ...
    for (int i = 0; i < refAlign->GetNumSequences(); i++) {
      ...
      for (int j = 0; !found && j < testAlign->GetNumSequences(); j++) {
        if (refAlign->GetSequence(i)->GetHeader() ==
            testAlign->GetSequence(j)->GetHeader())
          found = true;
      }
  ...
  if (useColScore) {
    ...
    if (testAlign)
      testCols = ComputeColumns(testAlign, false);
    set<VI> colIntersect;
    insert_iterator<set<VI>> colIntersectIter(colIntersect,
                                              colIntersect.begin());
    set_intersection(testCols.begin(), testCols.end(), refCols.begin(),
                     refCols.end(), colIntersectIter);
    TP = (int)colIntersect.size();
    TPFN = (int)refCols.size();
    if (testAlign)
      TPFP = (int)testCols.size();
  } else {
    refPairs = ComputePairs(refAlign, true);
    if (testAlign)
      testPairs = ComputePairs(testAlign, false);
    set<PII> pairIntersect;

    insert_iterator<set<PII>> pairIntersectIter(pairIntersect,
                                                pairIntersect.begin());
    set_intersection(testPairs.begin(), testPairs.end(), refPairs.begin(),
                     refPairs.end(), pairIntersectIter);
    TP = (int)pairIntersect.size();
    TPFN = (int)refPairs.size();
    if (testAlign)
      TPFP = (int)testPairs.size();
  }

  FD = (double)TP / TPFN;
  FM = (double)TP / TPFP;

  cout << GetName(string(argv[2])) << " " << TP << " " << TPFN << " " << TPFP
       << " " << FD << " " << FM << endl;
  ...
  for (int i = 1; i <= L; i++) {
    ...
    for (int j = 0; j < N - 1; j++) {
      for (int k = j + 1; k < N; k++) {
        ...
        if (makeAnnot && !isRef) {
          ct++;
          if (refPairs.find(p) != refPairs.end())
            good++;
        }
        ret.insert(p);
      }
