...
class ProbabilisticModel {
  ...
  float initialDistribution[NumMatrixTypes]; // holds the initial probabilities
                                             // for each state
  float transProb[NumMatrixTypes][NumMatrixTypes]; // holds all state-to-state
                                                   // transition probabilities
  float matchProb[256][256]; // emission probabilities for match states
  float insProb[256]
               [NumMatrixTypes]; // emission probabilities for insert states
...
public:
  ...
  ProbabilisticModel(const VF &initDistribMat, const VF &gapOpen,
                     const VF &gapExtend, const VVF &emitPairs,
                     const VF &emitSingle) {
  ...
  VF *ComputeForwardMatrix(Sequence *seq1, Sequence *seq2) const {
    ...
    // create matrix
    VF *forwardPtr =
        new VF(NumMatrixTypes * (seq1Length + 1) * (seq2Length + 1), LOG_ZERO);
    assert(forwardPtr);
  ...
  VF *ComputeBackwardMatrix(Sequence *seq1, Sequence *seq2) const {
    ...
    for (int k = 0; k < NumMatrixTypes; k++)
      backward[NumMatrixTypes * ((seq1Length + 1) * (seq2Length + 1) - 1) + k] =
    ...
    for (int i = seq1Length; i >= 0; i--) {
      ...
      for (int j = seq2Length; j >= 0; j--) {
        ...
        if (i < seq1Length) {
          for (int k = 0; k < NumInsertStates; k++) {
            LOG_PLUS_EQUALS(backward[0 + ij], backward[2 * k + 1 + i1j] +
                                                  insProb[c1][k] +
                                                  transProb[0][2 * k + 1]);
        ...
        if (j < seq2Length) {
          for (int k = 0; k < NumInsertStates; k++) {
            LOG_PLUS_EQUALS(backward[0 + ij], backward[2 * k + 2 + ij1] +
                                                  insProb[c2][k] +
                                                  transProb[0][2 * k + 2]);
  ...
  VF *ComputePosteriorMatrix(Sequence *seq1, Sequence *seq2, const VF &forward,
                             const VF &backward) const {
    ...
    float totalProb =
        ComputeTotalProbability(seq1Length, seq2Length, forward, backward);
  ...
  VVF *ComputeExpectedCounts () const {
    ...
    VVF *countsPtr = new VVF(NumMatrixTypes + 1, VF(NumMatrixTypes, LOG_ZERO));
  assert (countsPtr); VVF &counts = *countsPtr;
  ...
  */
  ...
  void ComputeNewParameters(Sequence *seq1, Sequence *seq2, const VF &forward,
                            const VF &backward, VF &initDistribMat, VF &gapOpen,
                            VF &gapExtend, VVF &emitPairs, VF &emitSingle,
                            bool enableTrainEmissions) const {
      ...
      for (int j = 0; j <= seq2Length; j++) {
        ...
        if (i > 0 && j > 0) {
          if (enableTrainEmissions && i == 1 && j == 1) {
            LOG_PLUS_EQUALS(pairCounts[c1][c2], initialDistribution[0] +
                                                    matchProb[c1][c2] +
                                                    backward[0 + ij]);
            LOG_PLUS_EQUALS(pairCounts[c2][c1], initialDistribution[0] +
                                                    matchProb[c2][c1] +
                                                    backward[0 + ij]);
          }
    ...
    initDistribMat[0] = min(
        1.0f, max(0.0f, (float)exp(initCounts[0]) / totalInitDistribCounts));
    for (int k = 0; k < NumInsertStates; k++) {
      float val = (exp(initCounts[2 * k + 1]) + exp(initCounts[2 * k + 2])) / 2;
      initDistribMat[2 * k + 1] = initDistribMat[2 * k + 2] =
          min(1.0f, max(0.0f, val / totalInitDistribCounts));
    }
    ...
    for (int i = 0; i < NumInsertStates; i++) {
      ...
      gapOpen[2 * i] = gapOpen[2 * i + 1] =
          (exp(transCounts[0][2 * i + 1]) + exp(transCounts[0][2 * i + 2])) /
        ...
        if (!islower((char)i)) {
          ...
          for (int j = 0; j <= i; j++)
            if (!islower((char)j)) {
              ...
              emitPairs[i][j] = emitPairs[i][lj] = emitPairs[li][j] =
                  emitPairs[li][lj] = emitPairs[j][i] = emitPairs[j][li] =
                      emitPairs[lj][i] = emitPairs[lj][li] =
                          exp(pairCounts[j][i]) / totalPairCounts;
            }
          emitSingle[i] = emitSingle[li] =
              exp(singleCounts[i]) / totalSingleCounts;
        }
    }
  }
  ...
  pair<SafeVector<char> *, float>
  ComputeAlignment(int seq1Length, int seq2Length, const VF &posterior) const {
  ...
  pair<SafeVector<char> *, float>
  ComputeAlignmentWithGapPenalties(MultiSequence *align1, MultiSequence *align2,
                                   const VF &posterior, int numSeqs1,
                                   int numSeqs2, float gapOpenPenalty,
    ...
    for (int i = 0; i <= seq2Length; i++) {
      ...
      oldRowInsertY[i] =
          (i == 0) ? 0
                   : oldRowInsertY[i - 1] + continuingPenalty2[numActive2[i]];
    ...
    for (int i = 1; i <= seq1Length; i++) {
      ...
      *(tracebackPtr) = *(tracebackPtr + 1) = *(tracebackPtr + 2) = 'X';
      ...
      for (int j = 1; j <= seq2Length; j++) {
        ...
        ChooseBestOfThree(
            oldRowMatch[j] + openingPenalty1[numActive1[i]][numGapOpens2[j]],
        ...
        ChooseBestOfThree(newRowMatch[j - 1] +
                          ...
                          newRowInsertX[j - 1] +
                              openingPenalty2[numActive2[j]][numGapOpens1[i]],
                          newRowInsertY[j - 1] +
                              continuingPenalty2[numActive2[j]],
  ...
  pair<SafeVector<char> *, float>
  ComputeViterbiAlignment(Sequence *seq1, Sequence *seq2) const {
    ...
    VI *tracebackPtr =
        new VI(NumMatrixTypes * (seq1Length + 1) * (seq2Length + 1), -1);
    assert(tracebackPtr);
    ...
    for (int i = 0; i <= seq1Length; i++) {
      ...
      for (int j = 0; j <= seq2Length; j++) {
        ...
        if (i > 0 && j > 0) {
          for (int k = 0; k < NumMatrixTypes; k++) {
            float newVal =
                viterbi[k + i1j1] + transProb[k][0] + matchProb[c1][c2];
        ...
        if (i > 0) {
          for (int k = 0; k < NumInsertStates; k++) {
            float valFromMatch =
                insProb[c1][k] + viterbi[0 + i1j] + transProb[0][2 * k + 1];
            float valFromIns = insProb[c1][k] + viterbi[2 * k + 1 + i1j] +
                               transProb[2 * k + 1][2 * k + 1];
        ...
        if (j > 0) {
          for (int k = 0; k < NumInsertStates; k++) {
            float valFromMatch =
                insProb[c2][k] + viterbi[0 + ij1] + transProb[0][2 * k + 2];
            float valFromIns = insProb[c2][k] + viterbi[2 * k + 2 + ij1] +
                               transProb[2 * k + 2][2 * k + 2];
            if (valFromMatch >= valFromIns) {
    ...
    while (r != 0 || c != 0) {
      int newState =
          traceback[state + NumMatrixTypes * (r * (seq2Length + 1) + c)];
  ...
  // where
  //                  [  P(s[i'] <--> t[j'])
  //                  [       if s[i'] is a letter in the ith column of align1
  //                  and [          t[j'] it a letter in the jth column of
  //                  align2
  //    f(s,t,i,j) =  [
  //                  [  0    otherwise
  //
  ...
  VF *
  BuildPosterior(MultiSequence *align1, MultiSequence *align2,
        ...
        if (first < second) {
          ...
          for (int jj = 1; jj <= matrix->GetSeq1Length(); jj++) {
            ...
            for (int ii = 0; ii < rowSize; ii++)
              posterior[base + (*mapping1)[row[ii].first] * (seq2Length + 1)] +=
                  row[ii].second;
