...
class TreeNode {
  ...
  void PrintNode(ostream &outfile, const MultiSequence *sequences) const {
    ...
    // otherwise, it must have two children; print out their subtrees
    // recursively
    else {
      assert(left);
      assert(right);
...
public:
  ...
  TreeNode(int sequenceLabel)
      : sequenceLabel(sequenceLabel), left(NULL), right(NULL), parent(NULL) {
  ...
  static TreeNode *ComputeTree(const VVF &distMatrix) {
    ...
    SafeVector<TreeNode *> nodes(numSeqs,
                                 NULL); // list of nodes for each sequence
      ...
      for (int i = 0; i < numSeqs; i++)
        if (valid[i]) {
          distances[bestPair.first][i] = distances[i][bestPair.first] =
              (distances[i][bestPair.first] + distances[i][bestPair.second]) *
              bestProb / 2;
        }
