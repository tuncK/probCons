...
MultiSequence *DoAlign(MultiSequence *sequence, const ProbabilisticModel &model,
                       VF &initDistrib, VF &gapOpen, VF &gapExtend,
...
int main(int argc, char **argv) {
  ...
  ReadParameters();
  PrintParameters("Using parameter set:", initDistrib, gapOpen, gapExtend,
                  emitPairs, emitSingle, NULL);
  ...
  if (enableTraining) {
    ...
    for (int i = 0; i < (int)initDistrib.size(); i++)
      initDistrib[i] = 0;
    for (int i = 0; i < (int)gapOpen.size(); i++)
      gapOpen[i] = 0;
    for (int i = 0; i < (int)gapExtend.size(); i++)
      gapExtend[i] = 0;
    if (enableTrainEmissions) {
      for (int i = 0; i < (int)emitPairs.size(); i++)
        for (int j = 0; j < (int)emitPairs[i].size(); j++)
          emitPairs[i][j] = 0;
      for (int i = 0; i < (int)emitSingle.size(); i++)
        emitSingle[i] = 0;
    }
    ...
    for (int i = 0; i < (int)sequenceNames.size(); i++) {
      ...
      // align sequences
      DoAlign(sequences, model, thisInitDistrib, thisGapOpen, thisGapExtend,
              thisEmitPairs, thisEmitSingle);

      // add in contribution of the derived parameters
      for (int i = 0; i < (int)initDistrib.size(); i++)
        initDistrib[i] += thisInitDistrib[i];
      for (int i = 0; i < (int)gapOpen.size(); i++)
        gapOpen[i] += thisGapOpen[i];
      for (int i = 0; i < (int)gapExtend.size(); i++)
        gapExtend[i] += thisGapExtend[i];
      if (enableTrainEmissions) {
        for (int i = 0; i < (int)emitPairs.size(); i++)
          for (int j = 0; j < (int)emitPairs[i].size(); j++)
            emitPairs[i][j] += thisEmitPairs[i][j];
        for (int i = 0; i < (int)emitSingle.size(); i++)
          emitSingle[i] += thisEmitSingle[i];
      }
    ...
    for (int i = 0; i < (int)initDistrib.size(); i++)
      initDistrib[i] /= (int)sequenceNames.size();
    for (int i = 0; i < (int)gapOpen.size(); i++)
      gapOpen[i] /= (int)sequenceNames.size();
    for (int i = 0; i < (int)gapExtend.size(); i++)
      gapExtend[i] /= (int)sequenceNames.size();
    if (enableTrainEmissions) {
      for (int i = 0; i < (int)emitPairs.size(); i++)
        for (int j = 0; j < (int)emitPairs[i].size(); j++)
          emitPairs[i][j] /= (int)sequenceNames.size();
      for (int i = 0; i < (int)emitSingle.size(); i++)
        emitSingle[i] /= sequenceNames.size();
    }

    PrintParameters("Trained parameter set:", initDistrib, gapOpen, gapExtend,
                    emitPairs, emitSingle, parametersOutputFilename.c_str());
  }
  ...
  // if we are not training, we must simply want to align some sequences
    ...
    for (int ct = 0; ct < numPreTrainingReps; ct++) {
      ...
      DoAlign(sequences, model, initDistrib, gapOpen, gapExtend, emitPairs,
              emitSingle);

      // print new parameters
      PrintParameters("Recomputed parameter set:", initDistrib, gapOpen,
...
void PrintParameters(const char *message, const VF &initDistrib,
  ...
  for (int i = 0; i < 5; i++){
    for (int j = 0; j <= i; j++){
      cerr << emitPairs[(unsigned char) alphabet[i]][(unsigned char)
  alphabet[j]] << " ";
    }
    cerr << endl;
    }*/
...
MultiSequence *DoAlign(MultiSequence *sequences,
                       const ProbabilisticModel &model, VF &initDistrib,
                       VF &gapOpen, VF &gapExtend, VVF &emitPairs,
                       VF &emitSingle) {
    ...
    for (int a = 0; a < numSeqs - 1; a++) {
      for (int b = a + 1; b < numSeqs; b++) {
        ...
        if (enableTraining) {
          ...
          model.ComputeNewParameters(seq1, seq2, *forward, *backward,
                                     thisInitDistrib, thisGapOpen,
                                     thisGapExtend, thisEmitPairs,
                                     thisEmitSingle, enableTrainEmissions);
          ...
          sparseMatrices[a][b] = new SparseMatrix(
          ...
          if (!enableAllPairs) {
            ...
            // compute "expected accuracy" distance for evolutionary tree
            // computation
            float distance =
  ...
  if (enableTraining) {
    ...
    // compute new parameters
    for (int i = 0; i < (int)initDistrib.size(); i++)
      initDistrib[i] /= numSeqs * (numSeqs - 1) / 2;
    for (int i = 0; i < (int)gapOpen.size(); i++)
      gapOpen[i] /= numSeqs * (numSeqs - 1) / 2;
    for (int i = 0; i < (int)gapExtend.size(); i++)
      gapExtend[i] /= numSeqs * (numSeqs - 1) / 2;

    if (enableTrainEmissions) {
      for (int i = 0; i < (int)emitPairs.size(); i++)
        for (int j = 0; j < (int)emitPairs[i].size(); j++)
          emitPairs[i][j] /= numSeqs * (numSeqs - 1) / 2;
      for (int i = 0; i < (int)emitSingle.size(); i++)
        emitSingle[i] /= numSeqs * (numSeqs - 1) / 2;
    }
  }
    ...
    if (enableAllPairs) {
      for (int a = 0; a < numSeqs - 1; a++) {
        for (int b = a + 1; b < numSeqs; b++) {
          ...
          if (enableVerbose)
            cerr << "Performing pairwise alignment: (" << a + 1 << ") "
                 << seq1->GetHeader() << " vs. "
                 << "(" << b + 1 << ") " << seq2->GetHeader() << " -- ";
          ...
          if (enableViterbi)
            ...
            VF *posterior = sparseMatrices[a][b]->GetPosterior();
            assert(posterior);
          ...
          string name = seq1->GetHeader() + "-" + seq2->GetHeader() +
                        (enableClustalWOutput ? ".aln" : ".fasta");
...
SafeVector<string> ParseParams(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    //         << "              subtract 0 <= CUTOFF <= 1 (default: " << cutoff
    //         << ") from all posterior values before final alignment" << endl
    //         << endl
...
void ReadParameters() {
  ...
  if (parametersInputFilename == string("")) {
    ...
    for (int i = 0; i < (int)alphabet.length(); i++) {
      ...
      for (int j = 0; j <= i; j++) {
        emitPairs[(unsigned char)tolower(alphabet[i])]
                 [(unsigned char)tolower(alphabet[j])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)tolower(alphabet[i])]
                 [(unsigned char)toupper(alphabet[j])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)toupper(alphabet[i])]
                 [(unsigned char)tolower(alphabet[j])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)toupper(alphabet[i])]
                 [(unsigned char)toupper(alphabet[j])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)tolower(alphabet[j])]
                 [(unsigned char)tolower(alphabet[i])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)tolower(alphabet[j])]
                 [(unsigned char)toupper(alphabet[i])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)toupper(alphabet[j])]
                 [(unsigned char)tolower(alphabet[i])] = emitPairsDefault[i][j];
        emitPairs[(unsigned char)toupper(alphabet[j])]
                 [(unsigned char)toupper(alphabet[i])] = emitPairsDefault[i][j];
      }
    ...
    for (int i = 0; i < 3; i++) {
      if (!getline(data, line[i])) {
        cerr << "ERROR: Unable to read transition parameters from parameter "
                "file: "
             << parametersInputFilename << endl;
        exit(1);
      }
    ...
    for (int i = 0; i < (int)alphabet.size(); i++) {
      for (int j = 0; j <= i; j++) {
        ...
        data >> val;
        emitPairs[(unsigned char)tolower(alphabet[i])]
                 [(unsigned char)tolower(alphabet[j])] = val;
        emitPairs[(unsigned char)tolower(alphabet[i])]
                 [(unsigned char)toupper(alphabet[j])] = val;
        emitPairs[(unsigned char)toupper(alphabet[i])]
                 [(unsigned char)tolower(alphabet[j])] = val;
        emitPairs[(unsigned char)toupper(alphabet[i])]
                 [(unsigned char)toupper(alphabet[j])] = val;
        emitPairs[(unsigned char)tolower(alphabet[j])]
                 [(unsigned char)tolower(alphabet[i])] = val;
        emitPairs[(unsigned char)tolower(alphabet[j])]
                 [(unsigned char)toupper(alphabet[i])] = val;
        emitPairs[(unsigned char)toupper(alphabet[j])]
                 [(unsigned char)tolower(alphabet[i])] = val;
        emitPairs[(unsigned char)toupper(alphabet[j])]
                 [(unsigned char)toupper(alphabet[i])] = val;
...
void TreeBasedBiPartitioning(
  ...
  // check if this is a node of the alignment tree
  if (tree->GetSequenceLabel() == -1) {
    TreeBasedBiPartitioning(sparseMatrices, model, alignment,
                            tree->GetLeftChild());
    TreeBasedBiPartitioning(sparseMatrices, model, alignment,
                            tree->GetRightChild());
    ...
    if (!leftSubtree.empty() && !leftSubtreeComplement.empty()) {
      ...
      assert(groupOneSeqs);
      MultiSequence *groupTwoSeqs = alignment->Project(leftSubtreeComplement);
      assert(groupTwoSeqs);
      delete alignment;
      alignment =
    ...
    if (!rightSubtree.empty() && !rightSubtreeComplement.empty()) {
      ...
      assert(groupOneSeqs);
      MultiSequence *groupTwoSeqs = alignment->Project(rightSubtreeComplement);
      assert(groupTwoSeqs);
      delete alignment;
      alignment =
          AlignAlignments(groupOneSeqs, groupTwoSeqs, sparseMatrices, model);
